               ...
               , dfd::carousels& carousels)
{
  ...
  dfd::program& program = programs.find_program(pmt_pid);
  ...
  typedef gts::dsmcc::download_data_block<typename user_network_type::base_iterator>
    download_data_block;
  download_data_block ddb(user_network);

  typedef typename download_data_block::download_id_iterator download_id_iterator;
  typedef typename download_data_block::module_id_iterator module_id_iterator;
  typedef typename download_data_block::module_version_iterator module_version_iterator;
  typedef typename download_data_block::block_number_iterator block_number_iterator;
  typedef typename download_data_block::block_data_iterator block_data_iterator;
            
  download_id_iterator download_id = gts::iterators::next<3>(ddb.begin ());
  std::cout << "DDB download_id: " << *download_id << std::endl;

  dfd::program::download_scenario_iterator download_scenario_iterator
    = program.download_scenarios.find(*download_id);
  if(download_scenario_iterator != program.download_scenarios.end())
  {
    dfd::download_scenario& ds = download_scenario_iterator->second;
    ...
    dfd::download_scenario::module_iterator module_iterator = ds.modules.find(*module_id);
    if(module_iterator != ds.modules.end())
    {
      dfd::module& module = module_iterator->second;
