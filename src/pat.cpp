...
void pat(unsigned char const* buf, std::size_t count, int fd, dfd::filters& filters
         , dfd::programs& programs, dfd::carousels& carousels)
{
  std::cout << "PAT packet" << std::endl;
  filters.remove_filter(fd);
  typedef gts::sections::program_association_section<unsigned char const*>
    pat_section_type;
  assert(count >= 4);
        ...
        ;first != last; ++first)
  {
    ...
    {
      dfd::filters::table_filter f(pa.pid);
      filters.add_table_filter(f, boost::bind(&dfd::parsers::nit, _1, _2, _3, boost::ref(filters))
                               , DMX_IMMEDIATE_START|DMX_CHECK_CRC);
    }
    else if(pa.program_number != 0)
    {
      dfd::filters::table_filter f(pa.pid);
      filters.add_table_filter(f, boost::bind(&dfd::parsers::pmt, _1, _2, _3, boost::ref(filters)
