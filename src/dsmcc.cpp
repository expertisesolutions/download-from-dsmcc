...
void dsmcc(unsigned char const* buf, std::size_t count, int fd, dfd::filters& filters
           ...
           , dfd::carousels& carousels)
{
  ...
  if(count >= private_section_length_iterator::bits_offset::value/CHAR_BIT)
  {
    ...
    case 0x3b:
    {
      dfd::parsers::dsmcc_un_messages(extended_section, fd, filters, programs, pmt_pid, dsmcc_pid);
    }
    case 0x3c:
    {
      dfd::parsers::dsmcc_ddb(extended_section, fd, filters, programs, pmt_pid, dsmcc_pid
                              , carousels);
    }
    break;
    }
